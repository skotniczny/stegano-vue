...
;(function (name, context, factory) {
  ...
  // Handle image url
    ...
    var modMessage = []
    var decM; var oldDec; var oldMask; var left; var right
    var dec; var curOverlapping; var mask
    ...
    // Write Data
    var offset; var index; var subOffset; var delimiter = messageDelimiter(modMessage, threshold)
    var q; var qS
    for (offset = 0; (offset + threshold) * 4 <= data.length && (offset + threshold) <= modMessage.length; offset += threshold) {
      ...
      for (i = 0; i < threshold && i + offset < modMessage.length; i += 1) {
        q = 0
        for (j = offset; j < threshold + offset && j < modMessage.length; j += 1) { q += modMessage[j] * Math.pow(args(i), j - offset) }
        qS[i] = (255 - prime + 1) + (q % prime)
      }
      for (i = offset * 4; i < (offset + qS.length) * 4 && i < data.length; i += 4) { data[i + 3] = qS[(i / 4) % threshold] }

      subOffset = qS.length
    }
    // Write message-delimiter
    for (index = (offset + subOffset); index - (offset + subOffset) < delimiter.length && (offset + delimiter.length) * 4 < data.length; index += 1) { data[(index * 4) + 3] = delimiter[index - (offset + subOffset)] }
  ...
  */ }
    ...
    var message = ''; var charCode = 0; var bitCount = 0; var mask = Math.pow(2, codeUnitSize) - 1
